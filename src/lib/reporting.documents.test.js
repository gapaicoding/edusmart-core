...
describe("Report Card PDF", () => {
  ...
  test("generates a real non-empty PDF with the bundled TrueType font", async () => {
    ...
    // The font remediation deliberately embeds a TrueType font instead of the
    // former WOFF webfont. With object streams disabled by the PDF generator,
    // these markers are inspectable in the produced bytes.
    const raw = Buffer.from(bytes).toString("latin1");
    expect(raw).toContain("/FontFile2");
    expect(raw).toContain("/ToUnicode");
  });

  test("font loader stays on the bundled Noto Sans TTF contract", async () => {
    const pdfSource = await readFile(new URL("./reporting.pdf.ts", import.meta.url), "utf8");
    const serverSource = await readFile(
      new URL("./reporting.documents.server.ts", import.meta.url),
      "utf8",
    );
    expect(pdfSource).toContain("subset: false");
    expect(serverSource).toContain("NotoSans-Regular.ttf");
    expect(serverSource).not.toContain("noto-sans-latin-ext-400-normal.woff");
  });
});
...
test("B8-LIVE-005 regeneration migration preserves authority while replacing metadata", async () => {
  const sql = await readFile(
    new URL(
      "../../supabase/migrations/20260911143000_b8_report_card_document_regeneration.sql",
      import.meta.url,
    ),
    "utf8",
  );

  expect(sql).toContain("create or replace function public.replace_report_card_document(");
  expect(sql).toContain("p_expected_file_asset_id uuid");
  expect(sql).toMatch(/from public\.report_cards[\s\S]*for update/i);
  expect(sql).toMatch(/from public\.generated_documents[\s\S]*for update of gd, fa/i);
  expect(sql).toContain("public.can_write_report_card_document_object(p_object_path)");
  expect(sql).toContain("delete from public.generated_documents");
  expect(sql).toContain("delete from public.file_assets");
  expect(sql).toContain("from public.register_report_card_document(");
  expect(sql).toContain("previous_object_path");
...
test("B8-LIVE-005 server and staff UI expose explicit regeneration without client authority", async () => {
  const functionsSource = await readFile(
    new URL("./reporting.documents.functions.ts", import.meta.url),
    "utf8",
  );
  ...
  expect(functionsSource).toContain("export const regenerateReportCardDocument");
  expect(functionsSource).toContain('"replace_report_card_document"');
  expect(functionsSource).toContain("p_expected_file_asset_id: existing.fileAssetId");
  expect(functionsSource).toContain("previous_object_path");
  expect(functionsSource).toContain("upsert: false");
  expect(functionsSource).not.toContain("SUPABASE_SERVICE_ROLE");
  expect(functionsSource).not.toContain("service_role");

  expect(uiSource).toContain("regenerateReportCardDocument");
  expect(uiSource).toContain("Regenerate PDF");
  expect(uiSource).toContain("Regenerate official PDF");
});

test("B8-LIVE-005 validator covers replacement RPC ACL and delegation contract", async () => {
  const validator = await readFile(
    new URL("../../supabase/validation/validate_b8_reporting_documents.sql", import.meta.url),
    "utf8",
  );
  expect(validator).toContain(
    "replace_report_card_document(uuid,uuid,text,bigint,text,bigint,text)",
  );
  expect(validator).toContain("B8 LIVE-005 regeneration contract");
  expect(validator).toContain("public.register_report_card_document");
  expect(validator).toContain("p_expected_file_asset_id");
});
