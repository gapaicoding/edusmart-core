...
import {
  assertReportCardPdfFontBytes,
  generateReportCardPdf,
  reportCardObjectPath,
  reportCardPdfFontFormat,
  reportCardScoreText,
  sha256Hex,
...
import {
  reportCardDocumentInput,
  portalReportCardDocumentInput,
...
const model = {
  ...
  subjects: [
    {
      ...
      narrative:
        "Menunjukkan perkembangan yang baik dalam membaca dan menulis.",
    },
  ...
  homeroomComment:
    "Catatan wali kelas: terus pertahankan semangat belajar.",
  ...
  narratives: [
    {
      ...
      content:
        "Batch 8 live reporting verification. Kehadiran dan catatan terbaca dengan baik.",
    },
  ],
};

describe(
  "Report Card document identity and policy model",
  () => {
    test(
      "uses attestation-bound random and version-isolated paths",
      () => {
        const base = {
          organizationId:
    ...
    test(
      "generation accepts published only",
      () => {
        expect(
          canGenerateReportCardDocument(
            "published",
    ...
    test(
      "metadata must bind exact Report Card, type, bucket, and path",
      () => {
        const base = {
          reportCardId:
            model.reportCardId,
    ...
    test(
      "signed links use a short fixed TTL",
      () => {
        expect(
          REPORT_CARD_SIGNED_URL_TTL_SECONDS,
        ).toBe(120);
    ...
    test(
      "download inputs reject unknown and tampered metadata identifiers",
      () => {
        expect(
...
describe(
  "trusted server document attestation",
  () => {
    const secret =
      "test-only-authority-secret-with-at-least-32-bytes";
    ...
    test(
      "accepts the exact server-generated contract",
      () => {
        expect(
          verifyReportCardDocumentAttestationForTest(
            input,
    ...
    test(
      "denies arbitrary PDF self-registration without trusted proof",
      async () => {
        const arbitrary =
          new TextEncoder().encode(
            "%PDF- arbitrary attacker bytes",
          );
    ...
    test(
      "binds bytes, checksum, size, Report Card, version, and path",
      () => {
        const changes = [
          {
            checksum:
    ...
    test(
      "denies expired or excessively future-dated proof",
      () => {
        expect(
          verifyReportCardDocumentAttestationForTest(
            input,
...
describe(
  ...
  () => {
    ...
    test(
      "preserves null versus zero score semantics",
      () => {
        expect(
          reportCardScoreText(
            null,
    ...
    test(
      "SHA-256 is deterministic over actual bytes",
      async () => {
        const bytes =
          new TextEncoder().encode(
            "pdf bytes",
    ...
    test(
      "accepts TTF/OTF and explicitly rejects the legacy WOFF input",
      async () => {
        const ttf =
          await loadTtfFont();
        ...
        expect(() =>
          ...
          /TrueType\/OpenType TTF or OTF/i,
        );
      },
    );

    test(
      "generates a readable Unicode-font PDF using realistic Indonesian text",
      async () => {
        const font =
          await loadTtfFont();
        ...
        expect(
          ...
          `${model.studentName} Report Card v${model.version}`,
        );

        /*
         * generateReportCardPdf saves without PDF object streams so these
         * font dictionaries are directly inspectable.
         *
         * FontFile2 = embedded TrueType font.
         * ToUnicode = Unicode character mapping used by PDF viewers.
         */
        const source =
          Buffer.from(
            bytes,
...
test(
  "R3 migration keeps Storage and function ACLs narrowly scoped",
  async () => {
    const migrationUrl =
      new URL(
    ...
    const r1 =
      await readFile(
        new URL(
          "../../supabase/migrations/20260907160000_b8_reporting_integrity.sql",
          import.meta.url,
        ),
        "utf8",
      );
    ...
    const validator =
      await readFile(
        new URL(
          "../../supabase/validation/validate_b8_reporting_documents.sql",
          import.meta.url,
        ),
        "utf8",
      );
    ...
    expect(
      ...
      /create policy[\s\S]*for update to authenticated[\s\S]*report-cards/i,
    );
    ...
    expect(
      ...
      "public.verify_report_card_document_attestation(",
    );
    ...
    expect(
      ...
      /length\(secret\)\s*>=\s*32/i,
    );

    expect(
      validator,
    ).toMatch(
      /length\(decrypted_secret\)\s*>=\s*32/i,
    );
    ...
    expect(
      ...
      "v_def ilike '%select secret into%'",
    );

    expect(
      validator,
    ).toContain(
      "v_def ilike '%min(ds.secret)%'",
    );
    ...
    expect(
      ...
      "public.can_access_report_card('report_card.download', rc.id)",
    );

    expect(
      r1,
    ).toContain(
      "sg.status='active' and sg.can_view_academic",
    );

    expect(
      sql,
    ).toContain(
      "v_rc.status <> 'published'",
    );
  },
);

test(
  "browser-facing document functions never return or read authority secrets",
  async () => {
    const functionsSource =
      await readFile(
        new URL(
          "./reporting.documents.functions.ts",
          import.meta.url,
        ),
        "utf8",
      );
    ...
    expect(
      ...
      /return\s*\{[^}]*attestation/is,
    );
  },
);

test(
  "R3 validator recognizes the escaped canonical path contract and rejects weakened variants",
  async () => {
    const validator =
      await readFile(
        new URL(
    ...
    expect(
      ...
      "position('/report-card\\.pdf$' in v_def) = 0",
    );
    ...
    const deployedShape =
      `p_object_path ~ '/report-cards/x/v[1-9][0-9]*/uuid/report-card\\.pdf$'
split_part(p_object_path, '/', 4)::uuid
split_part(p_object_path, '/', 6)::uuid
p_object_path = public.report_card_document_object_path
status = 'published'
...
const LIVE_002 = {
  organizationId:
    "a08b610f-9619-ad04-2d76-712dd0d7a537",

  schoolId:
    "8404d1b7-28be-a36a-9f84-c07d95d37527",

  reportCardV2Id:
    "9fa196ef-3eba-48cf-b890-2e69c24c8943",
...
const live002Path =
  (o) => {
    const p = {
      org:
        LIVE_002.organizationId,

      school:
        LIVE_002.schoolId,

      rc:
        LIVE_002.reportCardV2Id,

      v:
        "v2",

      gen:
        LIVE_002.generationV4,
...
describe(
  "B8-LIVE-002 canonical tenant document path contract",
  () => {
    test(
      "accepts valid PostgreSQL UUID tenant/report-card ids without RFC-version bits",
      () => {
        expect(
          CANONICAL_DOCUMENT_PATH.test(
            live002Path(),
          ),
        ).toBe(true);
    ...
    test(
      "keeps the strict UUIDv4 contract for the trusted generation nonce",
      () => {
        expect(
          CANONICAL_DOCUMENT_PATH.test(
            live002Path({
    ...
    test(
      "still rejects tampered tenant / card / version / filename / structure",
      () => {
        expect(
          CANONICAL_DOCUMENT_PATH.test(
            live002Path({
    ...
    test(
      "server path builder already agrees with the corrected canonical contract",
      () => {
        const built =
          reportCardObjectPath({
            organizationId:
...
test(
  "B8-LIVE-002 forward migration replaces path helpers without weakening authority",
  async () => {
    const sql =
      await readFile(
        new URL(
    ...
    expect(
      ...
      /drop\s+(function|policy|table)/i,
    );

    expect(
      sql,
    ).not.toMatch(
      /alter\s+table|create\s+table|create\s+policy/i,
    );
    ...
    expect(
      ...
      "4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}/report-card\\.pdf$",
    );

    expect(
      sql,
    ).not.toContain(
      "[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab]",
    );

    expect(
      sql,
    ).toContain(
      "p_object_path = public.report_card_document_object_path(rc.id",
    );
...
test(
  "B8-LIVE-002 validator detects regression to the over-strict tenant UUID regex",
  async () => {
    const validator =
      await readFile(
        new URL(
    ...
    expect(
      ...
      "register_report_card_document(uuid,text,bigint,text,bigint,text)",
    );
  },
);

test(
  "applied B8 migrations are byte-for-byte unchanged on this branch",
  async () => {
    const {
      execSync,
    } = await import(
