-- Read-only privilege-convergence validation for a populated database.
-- Transaction-level read-only enforcement replaces the fresh-bootstrap zero-row
-- invariant. External pre/post aggregate fingerprints prove this validation run
-- leaves selected domain table cardinalities unchanged.
...
declare
  ...
  control_plane_tables text[] := array[
    'organization_memberships','membership_school_access','roles','permissions',
    'role_permissions','membership_roles','invitations','audit_logs','file_assets',
    'generated_documents'
  ];
  item text;
  privilege_name text;
...
begin
  ...
  if has_table_privilege('authenticated', 'public.profiles', 'INSERT,UPDATE,DELETE')
     ...
     or not has_column_privilege('authenticated', 'public.profiles', 'avatar_file_id', 'UPDATE')
     or exists (
       select 1 from information_schema.columns c
       where c.table_schema='public' and c.table_name='profiles'
         and c.column_name <> all (array['full_name','phone','avatar_file_id'])
         and has_column_privilege('authenticated', 'public.profiles', c.column_name, 'UPDATE')
     ) then
    raise exception 'Authenticated profile update column restriction is invalid';
  end if;

  foreach item in array control_plane_tables loop
    if has_table_privilege('authenticated', format('public.%I', item), 'INSERT,UPDATE,DELETE') then
      raise exception 'Authenticated retains control-plane write privilege on public.%', item;
    end if;
  end loop;
  ...
  if current_setting('transaction_read_only') <> 'on' then
    raise exception 'Remote-safe privilege validation must run read-only';
  end if;
end
$privileges$;

select 'PRIVILEGE CONVERGENCE REMOTE-SAFE VALIDATION PASSED' as result;
