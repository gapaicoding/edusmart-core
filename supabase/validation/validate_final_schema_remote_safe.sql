-- Read-only validation of the final canonical B4 schema on a populated database.
...
begin
  ...
  if current_setting('transaction_read_only') <> 'on' then
    raise exception 'Remote-safe final schema validation must run read-only';
  end if;
end
$validation$;
...
select 'FINAL B4 REMOTE-SAFE SCHEMA VALIDATION PASSED' as result;
