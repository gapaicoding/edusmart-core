-- Forward-only fix for the conversion function's PL/pgSQL record/alias collision.
...
begin
 select app.*,cyc.status as cycle_status,cyc.academic_year_id as cycle_academic_year_id,cyc.name as cycle_name,yr.status as year_status
 into v_app
 from public.admission_applications app
 join public.admission_cycles cyc on cyc.id=app.admission_cycle_id and cyc.organization_id=app.organization_id and cyc.school_id=app.school_id
 join public.academic_years yr on yr.id=app.target_academic_year_id and yr.organization_id=app.organization_id and yr.school_id=app.school_id
 where app.id=p_application_id for update;
 ...
 for v_guardian in select * from public.admission_application_guardians ag where ag.application_id=v_app.id order by ag.is_primary desc,ag.created_at loop
   ...
   insert into public.student_guardians(organization_id,student_id,guardian_id,relationship_type,is_primary,status) values(v_app.organization_id,v_student,v_guardian_id,v_guardian.relationship,v_guardian.is_primary,'active');
