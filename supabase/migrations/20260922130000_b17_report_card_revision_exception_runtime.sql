-- Align the B17 historical-write exception with B16's actual in-flight
-- report-card command state ('started'). The B16 revision RPC completes its
-- ledger row only after the canonical revision routine has inserted its draft
-- and copied the snapshot rows. B15 score correction remains 'processing'.
  ...
  select exists(select 1 from public.audit_logs a where a.entity_type='report_card' and a.entity_id=p_card_id
    ...
    or exists(select 1 from public.report_card_command_requests c
      ...
      where c.actor_profile_id=auth.uid() and c.command_name='create_report_card_revision'
        and c.status='started' and mod(txid_current(),4294967296)=c.xmin::text::bigint
  ...
  if tg_table_name='report_cards' then
    ...
    if tg_op='INSERT' then
      v_allowed:=exists(select 1 from public.report_card_command_requests c join public.report_cards src on src.id=c.report_card_id
        where c.actor_profile_id=auth.uid() and c.command_name='create_report_card_revision' and c.status='started'
